...
1. Math at `../amari-chenstov-updates/mathematical_overview.ipynb` generalizes the REINFORCE algorithm to extend Frequentism through Information Geometric mechanics. The experiment concluded when a numerically intractable blocker was discovered. However, it coherently defines an online learning mechanism through single observation batches. Conveniently, it also has an optimal learning result, which we may leverage. Please store the result under `/notebooks/` and leverage its _single observation batches_ mechanic in this experiment's _phase 3_. 

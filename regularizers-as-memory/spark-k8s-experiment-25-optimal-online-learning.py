## Experiment 25: Real online learning but with approximately optimal lambda 
## 
## Hypothesis: online learning without restarting games works and is even better with hypothetically optimal lambda  
...
def map1(task_idx):
    ...
    try:
        ...
        condition_0_result_tuples_before = condition_0_model.simulate(total_iters=300, plot_prob_func=False, plot_rewards=False, silence_tqdm=True)
        ...
        ## apply natural regulaizer 
        ## lmbda = lambda model : model.hessian_denominator / 30 ## n_A / n_B ## srsly didn't work 
        condition_3_model.regularizing_lambda_function = lambda model : .5  
        condition_4_model.regularizing_lambda_function = lambda model : 2. 
        ## continue condition 0 trial, without application of memory and without discarding data
        condition_0_result_tuples_after = condition_0_model.simulate(total_iters=2*ITERS, plot_prob_func=False, plot_rewards=False, silence_tqdm=True)
        ## condition 1 (control): No use of memory, do discard data
        condition_1_model.clear_observations()
        condition_1_result_tuples_after = condition_1_model.simulate(total_iters=2*ITERS, plot_prob_func=False, plot_rewards=False, silence_tqdm=True)
        ## condition 2 (experimental): Use memory, do discard data
        condition_2_model.convert_observations_to_memory()
        condition_2_result_tuples_after = condition_2_model.simulate(total_iters=2*ITERS, plot_prob_func=False, plot_rewards=False, silence_tqdm=True)
        ## condition 3 (experimental): No use of memory, do discard data, drop high-info dims  
        condition_3_model.convert_observations_to_memory(keep_obs=200)  
        condition_3_result_tuples_after = condition_3_model.simulate(total_iters=2*ITERS, plot_prob_func=False, plot_rewards=False, silence_tqdm=True, memorization_frequency=200, keep_obs=200) 
        ## condition 4 (experimental): Use memory, do discard data, optimal lambda 
        condition_4_model.convert_observations_to_memory(keep_obs=100)
        condition_4_result_tuples_after = condition_4_model.simulate(total_iters=2*ITERS, plot_prob_func=False, plot_rewards=False, silence_tqdm=True, memorization_frequency=31, keep_obs=100) ## 31 > 30 = BATCH_SIZE 

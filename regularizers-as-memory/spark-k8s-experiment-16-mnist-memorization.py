## Experiment 16: illustrates memorization with dense nets on mnist  
...
def map1(task_idx):
    ...
    try:
        ...
        ## get data 
        mnist_initial_train_evens, mnist_initial_train_odds, _, _ = get_datasets(n=INITIAL_DATASET_SIZE) 
        mnist_train_evens, mnist_train_odds, mnist_test_evens, mnist_test_odds = get_datasets(n=SUBSAMPLE_SIZE) 
        ## define initial model  
        control_0_model = Model(net_type='dense', batch_norm=False, log1p_reg=False) ## only fit to windowed data, and don't memorize 
        ## initial fit 
        _ = control_0_model.fit(mnist_initial_train_evens, n_iters=FIT_ITERS, silence_tqdm=True, acc_frequency=ACC_FREQ, \
                even_label_test_set=mnist_test_evens, odd_label_test_set=mnist_test_odds)
        _ = control_0_model.fit(mnist_initial_train_odds, n_iters=FIT_ITERS, silence_tqdm=True, acc_frequency=ACC_FREQ, \
                even_label_test_set=mnist_test_evens, odd_label_test_set=mnist_test_odds)
        ## define other models 
        ...
        for idx in sampling_indices: 
            ...
            ## fit models 
            _ = control_0_model.fit(windowed_train_evens, n_iters=FIT_ITERS, silence_tqdm=True, acc_frequency=ACC_FREQ, l2_reg=L2_REG, \
                    even_label_test_set=mnist_test_evens, odd_label_test_set=mnist_test_odds) 
            _ = control_0_model.fit(windowed_train_odds, n_iters=FIT_ITERS, silence_tqdm=True, acc_frequency=ACC_FREQ, l2_reg=L2_REG, \
                    even_label_test_set=mnist_test_evens, odd_label_test_set=mnist_test_odds) 
            _ = control_1_model.fit(cumulative_train_evens, n_iters=FIT_ITERS, silence_tqdm=True, acc_frequency=ACC_FREQ, l2_reg=L2_REG, \
                    even_label_test_set=mnist_test_evens, odd_label_test_set=mnist_test_odds) 
            _ = control_1_model.fit(cumulative_train_odds, n_iters=FIT_ITERS, silence_tqdm=True, acc_frequency=ACC_FREQ, l2_reg=L2_REG, \
                    even_label_test_set=mnist_test_evens, odd_label_test_set=mnist_test_odds)
            idx_batch_even = experimental_model.fit(windowed_train_evens, n_iters=FIT_ITERS, silence_tqdm=True, l2_reg=L2_REG, \
                    acc_frequency=ACC_FREQ, ams=LAMBDA, even_label_test_set=mnist_test_evens, odd_label_test_set=mnist_test_odds) 
            experimental_model.memorize(windowed_train_evens, memorization_size=FIT_ITERS, silence_tqdm=True, krylov_rank=KRYLOV_RANK, \
                    krylov_eps=KRYLOV_EPS, idx_batch=idx_batch_even)
            idx_batch_odd = experimental_model.fit(windowed_train_odds, n_iters=FIT_ITERS, silence_tqdm=True, l2_reg=L2_REG, \
                    acc_frequency=ACC_FREQ, ams=LAMBDA, even_label_test_set=mnist_test_evens, odd_label_test_set=mnist_test_odds)
            experimental_model.memorize(windowed_train_odds, memorization_size=FIT_ITERS, silence_tqdm=True, krylov_rank=KRYLOV_RANK, \

...
def map1(task_idx):
    ...
    try:
        ...
        ## run experiment 
        from mnist_model import Model, mnist_train_evens_n10k, mnist_test_evens_n10k, mnist_train_odds_n10k, mnist_test_odds_n10k  
        ...
        sampling_indices = tqdm([idx*SAMPLING_STRIDE for idx in range(len(mnist_train_evens_n10k) // SAMPLING_STRIDE)]) 
        N = len(sampling_indices)  
        for idx in sampling_indices: 
            ## define training datasets 
            windowed_train_evens = mnist_train_evens_n10k[idx:(idx+SAMPLING_WINDOW)] 
            windowed_train_odds = mnist_train_odds_n10k[idx:(idx+SAMPLING_WINDOW)] 
            cumulative_train_evens = mnist_train_evens_n10k[:(idx+SAMPLING_WINDOW)] 
            cumulative_train_odds = mnist_train_odds_n10k[:(idx+SAMPLING_WINDOW)] 
            ## fit models 
            control_0_model.fit(windowed_train_evens, n_iters=FIT_ITERS, silence_tqdm=True, acc_frequency=ACC_FREQ, \
                    even_label_test_set=mnist_train_evens_n10k, odd_label_test_set=mnist_test_odds_n10k) 
        ...
        matric_5 = experimental_model.accs_high 

## Experiment 22: Actually optimal lambda  
## 
## Hypothesis: I found some math elluding to a decent lambda estimate  
...
def get_regularizing_lambda_function(model, prior_parameter_vector, multiplier=1.0): 
    ...
    lambda_estimate = nB * param_delta.transpose(0,1).matmul(hessian_estimate).matmul(param_delta) 
...
def map1(task_idx):
    ...
    try:
        ...
        ## condition 1 (experimental): Use memory, do dicard data, perhaps optimal lambda * 1.0  
        ...
        condition_1_model.regularizing_lambda_function = get_regularizing_lambda_function(condition_1_model, p, multiplier=1.0) 
        condition_1_model.convert_observations_to_memory()
        p = condition_1_model.get_parameter_vector().detach() 
        ...
        ## condition 2 (experimental): Use memory, do discard data, perhaps optimal lambda * 0.1  
